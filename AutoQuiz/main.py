...
from AutoQuiz.encoding.encoding import beam_search_decoding
from AutoQuiz.mcq.mcq import tokenize_sentences
from AutoQuiz.mcq.mcq import get_keywords
from AutoQuiz.mcq.mcq import get_sentences_for_keyword
from AutoQuiz.mcq.mcq import generate_questions_mcq
from AutoQuiz.mcq.mcq import generate_normal_questions
import time
...
class QGen:
    ...
    def __init__(self):
        ...
        model = T5ForConditionalGeneration.from_pretrained("C:\\8th semester\\Project\\AutoQuiz\\AQ-V2-para-Qgen\\results (1)\\aq-v1\\checkpoint-45000")
...
class BoolQGen:
    def __init__(self):
        ...
        model = T5ForConditionalGeneration.from_pretrained("C:\8th semester\Project\AutoQuiz\AQ-bool")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model.to(device)
        self.device = device
        self.model = model
...
class AnswerPredictor:
    ...
    def __init__(self):
        ...
        model = T5ForConditionalGeneration.from_pretrained("C:\8th semester\Project\AutoQuiz\AQ-boolean")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model.to(device)
        # model.eval()
        self.device = device
